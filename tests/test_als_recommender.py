...
class TestAlsRecommender(unittest.TestCase):
    ...
    def test_build_confidence_matrix_excludes_specified_pairs(self):
        ratings = pd.DataFrame(
            [
                {"userId": 10, "movieId": 1, "rating": 5.0},
        ...
        values = sorted(user_items.data.tolist())
        self.assertEqual(values, [21.0, 41.0])

    def test_build_confidence_matrix_none_exclude_is_no_op(self):
        ratings = pd.DataFrame(
            [
                {"userId": 10, "movieId": 1, "rating": 5.0},
                {"userId": 11, "movieId": 3, "rating": 4.0},
        ...
        baseline = build_confidence_matrix(ratings, positive_threshold=4.0)
        none_result = build_confidence_matrix(ratings, positive_threshold=4.0, exclude_pairs=None)
        empty_result = build_confidence_matrix(ratings, positive_threshold=4.0, exclude_pairs=set())
